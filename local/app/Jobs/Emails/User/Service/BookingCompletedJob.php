...
class BookingCompletedJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $subject = \Lang::choice('emails.user.booking_completed', null, ['booking_code' => $booking->booking_code], $locale);

            \App\Jobs\SendEmailJob::dispatch('user.service.booking_completed', $subject, $email, $emailArr);
        } catch (\Exception $e) {
            \Log::error($e);
        } finally {
            return true;
