...
class BookingCancelledJob implements ShouldQueue
{
    ...
    public function handle()
    {
        try {
            ...
            $item_name = \App\Models\ServiceBookingDetail::where('service_booking_id', $booking->id)->value("{$lang}name");
            ...
            $subject = \Lang::choice('emails.user.booking_cancelled', null, ['booking_code' => $booking->booking_code], $locale);

            \App\Jobs\SendEmailJob::dispatch('vendor.service.booking_cancelled', $subject, $email, $emailArr);
        } catch (\Exception $e) {
            \Log::error($e);
        } finally {
            return true;
