...
@section('title') {{ transLang('all_product_bookings') }} @endsection

@section('content')
    <section class="content-header">
        <ol class="breadcrumb">
            <li><a href="{{ route('admin.dashboard') }}"><i class="fa fa-dashboard"></i> {{ transLang('dashboard') }}</a></li>
            <li class="active">{{ transLang('all_product_bookings') }}</li>
        </ol>
    </section>
            ...
            <div class="col-md-12">
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">{{ transLang('all_product_bookings') }}</h3>
                    </div>
                    <div class="box-body">
                        <div class="row">
                            <div class="col-sm-3">  
                                <label class="control-label">{{ transLang('product') }}</label>
                                <div class="form-group">
                                    <select name="product_id" class="form-control select2-class2 reload-tbl" data-placeholder="{{ transLang('choose') }}">
                                        <option value=""></option>
                                        @if ($products->count())
                            ...
                            <div class="col-sm-3"> 
                                <label class="control-label">{{ transLang('booking_status') }}</label> 
                                 <div class="form-group">
                                    <select name="booking_status" class="form-control select2-class2 reload-tbl" data-placeholder="{{ transLang('choose') }}">
                                        <option value=""></option>
                                        @foreach (transLang('product_booking_status_arr') as $key => $val)
                            ...
                            <div class="col-sm-3"> 
                                <label class="control-label">{{ transLang('payment_status') }}</label> 
                                 <div class="form-group">
                                    <select name="payment_status" class="form-control select2-class2 reload-tbl" data-placeholder="{{ transLang('choose') }}">
                                        <option value=""></option>
                                        @foreach (transLang('payment_status_arr') as $key=> $payment_status)
                                            <option value="{{ $key }}">{{ $payment_status }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div> 
            ...
            $('#data-table').DataTable({
                ...
                ajax: {
                    ...
                    data: function (params) {
                        params.product_id = $('[name="product_id"]').val();
                    ...
                    @if (hasPermission('admin.product_bookings.view'))
                        {
                            ...
                            mRender: (data, type, row) => `<a href="{{ route('admin.product_bookings.view') }}/${row.id}">${data}</a>`
                        },
                    @else
                        ...
                        mRender: (data, type, row) => {
                            return `
                                ...
                                @if (hasPermission('admin.product_bookings.delete'))
                                    <a href="{{ route('admin.product_bookings.delete') }}/${row.id}" class="delete-entry" data-tbl="data"><i
